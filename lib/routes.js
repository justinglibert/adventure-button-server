...
var gameModes = new Set(["Question"]);
var questions = new Set(["Do you want to be Hokage?"]);
var questionToProperty = new Map([["Do you want to be Hokage?",(location) => {location.country == "Japan"}]])
  ...
  return new Promise(function(resolve, reject) {
    switch (action.name) {
      ...
      case "startGame": {
        ...
        internalState.gameSpecific = gameInit(internalState.mode)
        broadcast();
        resolve();
        break;
      }
      ...
      case "nextMinigame": {
        ...
        if (internalState.started.size == internalState.players.length) {
          internalState = {...oldState,
            location : updateLocations(oldState.locations),
            mode : nextGame()
          }
          internalState.gameSpecific = gameInit(internalState.mode)
        }
...
function nextGame() {
  ...
  serverLink.log('mfff', internalState)
  if (chosenGame != "Question") {
    gameModes.delete(chosenGame);
...
function getQuestion() {
  ...
  let chosenQuestion =
    questionArray[Math.floor(Math.random() * questions.size)];
...
function gameInit(mode) {
  switch (mode) {
    case "Question": {
      return { question: getQuestion(), responses: new Map() };
